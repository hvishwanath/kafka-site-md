...
![](/39/images/streams-interactive-queries-03.png)
...
![](/39/images/streams-interactive-queries-api-01.png)
...
After the application has started, you can get access to "CountsKeyValueStore" and then query it via the [ReadOnlyKeyValueStore](https://github.com/apache/kafka/blob/3.9/streams/src/main/java/org/apache/kafka/streams/state/ReadOnlyKeyValueStore.java) API:
...
After the application has started, you can get access to "CountsWindowStore" and then query it via the [ReadOnlyWindowStore](https://github.com/apache/kafka/blob/3.9/streams/src/main/java/org/apache/kafka/streams/state/ReadOnlyWindowStore.java) API:
...
To make this store queryable you must:
  ...
  * Provide an implementation of [QueryableStoreType](https://github.com/apache/kafka/blob/3.9/streams/src/main/java/org/apache/kafka/streams/state/QueryableStoreType.java).
...
![](/39/images/streams-interactive-queries-api-02.png)
...
The following methods return [StreamsMetadata](/39/javadoc/org/apache/kafka/streams/state/StreamsMetadata.html) objects, which provide meta-information about application instances such as their RPC endpoint and locally available state stores.
...
If `application.server` is not configured for an application instance, then the above methods will not find any [StreamsMetadata](/39/javadoc/org/apache/kafka/streams/state/StreamsMetadata.html) for it.
...
[Previous](/39/documentation/streams/developer-guide/testing) [Next](/39/documentation/streams/developer-guide/memory-mgmt)
